...
import { 
  onAuthStateChanged, signOut, doc, getDoc, setDoc, auth, db, serverTimestamp, collection, query, onSnapshot, orderBy 
} from './lib/firebase';
import { APP_THEMES, GLOBAL_STYLES, CUSTOM_STYLES, generateMockHistory, INITIAL_EVENTS, INITIAL_TASKS, INITIAL_COURSES } from './constants';
...
export default function App() {
  ...
  const [courses, setCourses] = useState<Course[]>(INITIAL_COURSES); // Shared Course State
  ...
  // --- TASK PERSISTENCE LOGIC (FIRESTORE) ---
  useEffect(() => {
    if (!currentUser) {
      setTasks([]);
      return;
  ...
  // --- COURSES PERSISTENCE LOGIC (FIRESTORE) ---
  useEffect(() => {
    if (!currentUser) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      // Don't duplicate AI tasks if already present recently
      ...
      if (brainResponse.suggestedTask && !tasks.some(t => t.text === brainResponse.suggestedTask!.text)) {
        // Add suggested task to Firestore instead of just local state
        const taskRef = collection(db, 'users', currentUser.uid, 'tasks');
        await setDoc(doc(taskRef), {
          ...brainResponse.suggestedTask,
          createdAt: Date.now(),
          order: tasks.length
        });
      }
    }, 5000); // Slightly more delay for stability

    return () => clearTimeout(timer);
  }, [tasks, userData, events, view, currentUser]); 
  // ---------------------------------
