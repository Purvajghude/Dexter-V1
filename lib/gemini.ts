...
export async function consultTheBrain(
  ...
  const prompt = `
    You are the "Brain" of Dexter, a Solarpunk cognitive optimizer. 
  ...
  try {
    ...
    // response.text is a property, not a method, and returns the generated string
    ...
    let fallbackInsight = "Offline mode. Trust your intuition.";
    if (error.message?.includes('429')) fallbackInsight = "Neural network congested (High Traffic). Operating on local heuristics.";
    if (error.message?.includes('404')) fallbackInsight = "Cognitive model not found (404). Check API config.";
